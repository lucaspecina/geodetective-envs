...
>
> **Sesión autónoma 2026-07-13 (tarde-noche)**: codebook v1.2 con sign-off de Codex (R5-R7). **Batería de probes COMPLETA e implementada** (`src/geodetective/probes.py`, 37 tests, smokes E017 validados, 2 bugs cazados): P1 contradicción (certificado/fuente débil 55-45/placebo source-matched, report inmediato forzado), P4 falsa verificación (assigned-test, exposición exógena), P5 distractor temporal (diff-in-diff). Smokes: mini suelta ante certificado (residual 0.0) pero overreacciona al rumor (elasticidad 3.15, H2 viva), no importa año declarado ajeno (P5≈0), honesto ante chequeo inútil (P4: inconclusive 3/3). **Perfilador conductual capa-2** (`scripts/behavior_profile.py`, ~35 señales mecánicas sin intervención, auditado R7): opus busca en idioma local 41% vs mini 13%, casi sin callejones sin salida (0.1 vs 0.9/corrida), 0% mismatch dicho-hecho vs 10%. **Dossier HTML** (`scripts/build_dossier.py`): análisis + expedientes paso a paso con imágenes — ver README "Dossier". Gates pre-confirmatorio: G1 (elegibilidad C/W en corpus nuevo), G2 (potencia), G3 (serialización prefijos), G5 (sign-off + tag freeze). G4 ✅.
>
...
**Junio 2026 — pivote belief-state en curso (E016, #47)**: el reward principal pasa de distancia puntual a **proper scoring rules sobre distribuciones de creencia** (mezcla vMF geodésica + reward denso por paso = ganancia de información verificada, sin LLM judge). Diseño completo en `research/synthesis/belief_state_redesign.md`. Implementado: scorer (`src/geodetective/eval/belief_scoring.py`) + tests sintéticos (`scripts/test_belief_scoring.py`, 9/9 OK — incluye properness por Monte Carlo y la divergencia log-score vs energy score ante el confiado-equivocado); belief-mode en el scaffold (`react.py`: tool `report_belief` + `evidence_chain` en submit + nudge, todo detrás de `belief_mode=True` — el brazo OFF queda idéntico al canónico; sin scoring en runtime, ground truth no entra al loop; tests en `scripts/test_belief_tool.py`). Budget económico v1 en scaffold (`tool_budget` opt-in, cobro por invocación, report_belief/submit gratis, bloqueo al agotarse). **Smoke tests validados** (gpt-5.4-mini): Estocolmo 1916 → 1.5 km con curva que expone lock-in narrativo en datación (web evidence arregló ubicación y contaminó año); Montevideo 1907 con budget=25 → 0.1 km, geocodes baratos como test discriminante, bloqueo y cierre graceful. 10 fotos pilot seleccionadas (`experiments/E016_belief_pilot/pilot_photos.json`, 1 por bucket × 6 décadas). **Belief trajectory viewer** construido (`scripts/build_belief_viewer.py`): selector de trayectorias, timeline step-a-step completo (thinking, tools, observaciones, imágenes, beliefs) + mapa Leaflet sticky con la creencia vigente por step, curva de info ganada y belief de año. Certificaciones hindsight DRAFT de las 10 fotos pilot en `pilot_photos.json`. Pendiente: validación user de las certificaciones, corridas pilot 3 modelos × N=3 × {on, off}, verificador de claims.

**Harness review + fixes masivos (2026-06-12, `research/notes/harness_review_post_belief.md`)**: auditoría crítica del harness encontró y arregló 13 bugs — los graves: `image_search._do_pick` crasheaba siempre desde el refactor #45 (modo zoom muerto en producción), páginas 2+ eran grillas negras con celdas impickeables, DDG devuelve ~1 resultado para queries largas (fallback transparente agregado), `invalid_submit` no terminaba el episodio (tool_calls huérfanas → 400), `street_view nearby` prometido en prompt pero jamás cableado, default real de web_search 5 vs 10 prometido. Además: early-block de `site:` bloqueado (ahorra calls), notas explicativas de blacklist al modelo, helper de web_search en modo extractivo (citas literales). ⚠️ La conclusión histórica "tools visuales infrautilizadas" está parcialmente confundida por harness roto — re-evaluar post-pilot. Wave 1 del pilot (gpt-5.4-mini × 10 × N=1 × on/off) corrió con harness viejo: sirve como sanity (mecánica end-to-end OK), se descarta y re-corre para el pilot.
